  ...
  <main class="main container">
      ...
      <section class="poster-gallery">
        <article class="poster">
          ...
          <article class="poster">
          <img src="${k}" srcset="${$} 2x, ${H} 3x" alt="poster-2">
          <h3 class="poster-title" data-i18n="Exclusive <br>Styles">Exclusive Styles</h3>
        </article>
        <article class="poster">
          <img src="${I}" srcset="${P} 2x, ${B} 3x" alt="poster-3">
...
`;const V="modulepreload",M=function(n){return"/iphone-banner/"+n},w={},c=function(s,r,o){let e=Promise.resolve();if(r&&r.length>0){const t=document.getElementsByTagName("link"),i=document.querySelector("meta[property=csp-nonce]"),d=(i==null?void 0:i.nonce)||(i==null?void 0:i.getAttribute("nonce"));e=Promise.all(r.map(a=>{if(a=M(a),a in w)return;w[a]=!0;const p=a.endsWith(".css"),L=p?'[rel="stylesheet"]':"";if(!!o)for(let u=t.length-1;u>=0;u--){const g=t[u];if(g.href===a&&(!p||g.rel==="stylesheet"))return}else if(document.querySelector(`link[href="${a}"]${L}`))return;const l=document.createElement("link");if(l.rel=p?"stylesheet":V,p||(l.as="script",l.crossOrigin=""),l.href=a,d&&l.setAttribute("nonce",d),document.head.appendChild(l),p)return new Promise((u,g)=>{l.addEventListener("load",u),l.addEventListener("error",()=>g(new Error(`Unable to preload CSS for ${a}`)))})}))}return e.then(()=>s()).catch(t=>{const i=new Event("vite:preloadError",{cancelable:!0});if(i.payload=t,window.dispatchEvent(i),!i.defaultPrevented)throw t})},O=(n,s)=>{const r=n[s];return r?typeof r=="function"?r():Promise.resolve(r):new Promise((o,e)=>{(typeof queueMicrotask=="function"?queueMicrotask:setTimeout)(e.bind(null,new Error("Unknown variable dynamic import: "+s)))})},C=["de","en","es","fr","ja","pt"],y=navigator.language.split("-")[0],m=new URLSearchParams(window.location.search).get("lang"),T=async n=>{try{return(await O(Object.assign({"./shared/languages/de.json":()=>c(()=>import("./de-B2zBMGoz.js"),[]),"./shared/languages/en.json":()=>c(()=>import("./en-Dk_vXuVS.js"),[]),"./shared/languages/es.json":()=>c(()=>import("./es-B1cSBe2h.js"),[]),"./shared/languages/fr.json":()=>c(()=>import("./fr-I1bC62mN.js"),[]),"./shared/languages/ja.json":()=>c(()=>import("./ja-nwj-E3O8.js"),[]),"./shared/languages/pt.json":()=>c(()=>import("./pt-D-4C-CmV.js"),[])}),`./shared/languages/${n}.json`)).default}catch(s){return console.error("Failed to load language file:",s),null}};async function f(n){const s=await T(n),r={de:{".main-title":"2.69rem",".btn-note":"1rem",".nav-link":"0.9rem"},es:{".nav-link":"1.2rem"},fr:{".main-title":"3.7rem",".btn-note":"1rem",".btn-title":"1.5rem",".btn-subtitle":"1.5rem",".nav-link":"1rem"},pt:{".btn-note":"1rem",".nav-link":"1.2rem"}};document.querySelectorAll("[data-i18n]").forEach(e=>{const t=e.getAttribute("data-i18n");if(t&&s[t]){const i=s[t],d=e.getAttribute("data-price");e.innerHTML=d?i.replace(/{{price}}/,d):i}});const o=r[n];o&&Object.entries(o).forEach(([e,t])=>{document.querySelectorAll(e).forEach(i=>{i.style.fontSize=t})})}function R(){(m==null?void 0:m.length)===2&&C.includes(m)?f(m):C.includes(y)?f(y):f("en")}R();document.getElementById("option-1").addEventListener("click",function(){const n=document.getElementById("option-1"),s=document.getElementById("option-2");if(n&&s){s.classList.remove("active"),n.classList.add("active");const r=n.getAttribute("data-href");r&&localStorage.setItem("selectedHref",r)}});document.getElementById("option-2").addEventListener("click",function(){const n=document.getElementById("option-1"),s=document.getElementById("option-2");if(n&&s){n.classList.remove("active"),s.classList.add("active");const r=s.getAttribute("data-href");r&&localStorage.setItem("selectedHref",r)}});document.getElementById("submit").addEventListener("click",function(){const n=localStorage.getItem("selectedHref");n&&window.open(n,"_blank")});
